...
import { Atom, BookOpenText, Calculator, Code, FlaskConical, Globe, HeartPulse, Microscope, Palette, Scale, Wallet } from 'lucide-react';
...
const getIcon = (category: Department['category'], name: string) => {
  ...
  switch (category) {
    case 'Sciences': return <FlaskConical className="h-6 w-6 text-teal-500" />;
    case 'Engineering': return <Atom className="h-6 w-6 text-blue-600" />;
    case 'Arts & Humanities': return <BookOpenText className="h-6 w-6 text-amber-600" />;
    default: return <Scale className="h-6 w-6 text-slate-500" />;
  }
};
