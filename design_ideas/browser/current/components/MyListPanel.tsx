...
import { X, Sparkles, ArrowRight, Share2, Download, Search, Layers, GripVertical } from 'lucide-react';
...
const MyListPanel: React.FC<MyListPanelProps> = ({
  isOpen,
  onClose,
  savedCourses,
  width = DEFAULT_WIDTH,
  ...
  return (
    <>
      ...
      <div
        className={`
          fixed inset-0 z-20 bg-black/30 backdrop-blur-sm
      ...
      <div
        ...
        className={`
          ...
          ${isOpen
            ...
            : 'animate-slideOutRight pointer-events-none'
          }
        `}
        style={{
          width: `${width}px`,
      ...
      >
        ...
        <div
          className={`
            ...
            ${isHoveringResizeHandle || isResizing ? 'bg-indigo-500/30' : 'hover:bg-white/10'}
          `}
        ...
        <div
          className={`
            h-20 flex items-center justify-between px-6 border-b border-white/5 bg-slate-900/40 shrink-0
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          ...
          {savedCourses.length === 0 ? (
            <div
              className={`h-full flex flex-col items-center justify-center text-center space-y-6 ${isOpen ? 'animate-fadeInUp' : ''}`}
            ...
            <div className="space-y-8">
              ...
              {/* Dark Search Bar */}
              <div
                className={`relative group z-10 ${isOpen ? 'animate-fadeInUp' : ''}`}
              ...
              <div
                className={`relative group perspective-1000 ${isOpen ? 'animate-fadeInUp' : ''}`}
                style={{ animationDelay: '200ms', animationFillMode: 'both' }}
              >
              ...
              {/* Course List */}
              <div
                className={`space-y-8 ${isOpen ? 'animate-fadeInUp' : ''}`}
                style={{ animationDelay: '250ms', animationFillMode: 'both' }}
              >
                {(Object.entries(groupedCourses) as [string, Course[]][]).map(([department, courses]) => (
        ...
        {/* Panel Footer */}
        <div
          className={`p-6 border-t border-white/10 bg-black/20 backdrop-blur-md space-y-4 shrink-0 z-10 ${isOpen ? 'animate-fadeInUp' : ''}`}
